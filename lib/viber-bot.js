...
ViberBot.prototype._setupConversationStartedHandler = function() {
	const self = this;
	this.on(EventConsts.CONVERSATION_STARTED, (context, response) => {
...
ViberBot.prototype._handleEventReceived = function(data) {
	...
	switch (data.event) {
		...
		case EventConsts.CONVERSATION_STARTED: {
			const response = new Response(this, userProfile);
			if(!data.context) const context = null;
			else const context = data.context;
			this.emit(data.event, response, context);
			break;
		}
