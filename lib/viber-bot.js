...
ViberBot.prototype._setupConversationStartedHandler = function() {
	const self = this;
	this.on(EventConsts.CONVERSATION_STARTED, (response, context) => {
		_.each(self._callbacks[EventConsts.CONVERSATION_STARTED], callback => {
			if (!response.userProfile) throw new Error("Response from conversation started callback must be message or null");
			if (!context) throw new Error("Response from conversation started callback must be message or null");
			return(response.userProfile, context);			
		});
	});
};
...
ViberBot.prototype._handleEventReceived = function(data) {
	...
	switch (data.event) {
		...
		case EventConsts.CONVERSATION_STARTED: {
			const response = new Response(this, userProfile);
			this.emit(data.event, response, data.context);
			break;
		}
